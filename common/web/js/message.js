Message = new function () {
    ...
    this.sendDataForm = function(domObject){
        var $this = $(domObject),
            link = $this.attr('data-link'),
            input_container = $this.attr('data-input')

        var form = $(input_container)[0],
            form_data = new FormData(form);

        Message.validation_errors_class = 'validation';
        Message.validation_errors_container = $(input_container + ' .'+Message.validation_errors_class);
...
//            timeout: 600000,
            success: function (data) {
                data.map(function (elem) {
                    try {
                        that.handleMessage.call(that, elem);
                    } catch (e) {
                        console.log('error in handling message', e);
                    }
                });
                ...
                if (data[0].type !== 'show_validation_errors' // Это не ответ о непройденной валидации отправленных данных
                    && data[0].type !== 'error'
                    && data[0].type !== 'show_validation_errors_input' // Это не сообщение об ошибке
                    && (typeof(data[0].data) !== 'string' || data[0].data.indexOf('Navigation.updateCoreInfo(true)') === -1) // Это не ошибка ключа безопасности
                    && input_container !== false) {
                    Core.clearInputData(input_container);
                }
...
Core.onFullLoad(function () {
    ...
    $('body').on('click','.senddata-form',function(){
        var domObject = this;

        Message.sendDataForm(domObject);
    });
...
Message.registerHandler('show_validation_errors_input', function (data) {
    ...
    $.each(data, function(input_name, messages) {
        ...
        var input = Message.validation_errors_container.find(':input[id $="-'+input_name+'"],:input[id="'+input_name+'"],:input[name="'+input_name+'"]');
//        var input = Message.validation_errors_container.find('#'+input_name);
        ...
        if (input.closest('.'+Message.validation_errors_class).find('.input-group').length > 0){
            input = input.closest('.'+Message.validation_errors_class).find('.input-group')
        }
        ...
        if (first){
            ...
            var destination = input.offset().top - 10;
